...
Rp = 0.25
#求坠落角（苏雨）    
gamma = math.atan(H/(v_cruise*sqrt((2*m)/(row*CD*A*g))*math.acosh(math.exp((row*CD*A*H)/(2*m)))))
#求坠落影响面积（苏雨）
A_rotorcraft = pi*(L_aricraft/2 + Hp/(2*math.tan(gamma)) + Rp)**2

#开始计算各个区域行人风险（苏雨）
num = 0
while num < len(shelterlist):
    R_people = wpeople*shelterlist[num]*Ek*rowpeople*peopleweight[num]*A_rotorcraft*P_crash
    Peoplelist.append(R_people)
    num = num + 1
#print(Peoplelist)
...
colorcountlist = []
peoplenum = 0
while peoplenum < (len(Peoplelist)):
    #按照不同道路风险绘制风险分布图（苏雨）
    if Peoplelist[peoplenum] > 225:
        color_count = len(color_list)
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 210:
        color_count = len(color_list) - 1
        colorcountlist.append(color_count) 
    elif Peoplelist[peoplenum] > 195:
        color_count = len(color_list) - 2 
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 180:
        color_count = len(color_list) - 3 
        colorcountlist.append(color_count)              
    elif Peoplelist[peoplenum] > 165:
        color_count = len(color_list) - 4 
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 150:
        color_count = len(color_list) - 5 
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 135:
        color_count = len(color_list) - 6 
        colorcountlist.append(color_count)       
    elif Peoplelist[peoplenum] > 120:
        color_count = len(color_list) - 7 
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 105:
        color_count = len(color_list) - 8 
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 90:
        color_count = len(color_list) - 9 
        colorcountlist.append(color_count)              
    elif Peoplelist[peoplenum] > 75:
        color_count = len(color_list) - 10
        colorcountlist.append(color_count) 
    elif Peoplelist[peoplenum] > 60:
        color_count = len(color_list) - 11
        colorcountlist.append(color_count) 
    elif Peoplelist[peoplenum] > 45:
        color_count = len(color_list) - 12
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 30:
        color_count = len(color_list) - 13
        colorcountlist.append(color_count)
    elif Peoplelist[peoplenum] > 15:
        color_count = len(color_list) - 14
        colorcountlist.append(color_count)
    else:
        color_count = len(color_list) - 15
        colorcountlist.append(color_count)  
    peoplenum = peoplenum + 1

print(colorcountlist)
...
while foldernum < len(windarea):
    ...
    while areanum < len(windarea[foldernum]):
        ...
        #这里开始给地图插入行人风险（苏雨）
        #print(namelist[foldernum])
        if (namelist[foldernum] == 'High Street and Promenades'):
            f.write('\t\t\t<styleUrl>'+ '#default7' + '</styleUrl>' + '\n')      
        elif (namelist[foldernum] == 'Railway Buzz'):
            f.write('\t\t\t<styleUrl>'+ '#default8' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'Waterside Settings'):
            f.write('\t\t\t<styleUrl>'+ '#default9' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'Countryside Sceneries'):
            f.write('\t\t\t<styleUrl>'+ '#default7' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'Suburban Landscapes'):
            f.write('\t\t\t<styleUrl>'+ '#default9' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'The Old Town'):
            f.write('\t\t\t<styleUrl>'+ '#default1' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'Victorian Terraces'):
            f.write('\t\t\t<styleUrl>'+ '#default3' + '</styleUrl>' + '\n')         
        elif (namelist[foldernum] == 'Central Business District'):
            f.write('\t\t\t<styleUrl>'+ '#default0' + '</styleUrl>' + '\n') 
